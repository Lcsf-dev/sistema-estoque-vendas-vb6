...
Private Sub Pesquisar()
    ...
    Set rs = Consultar("dbo.usp_FornecedoresListar", "@Busca", Trim$(txtBusca.Text), "@IncluirInativos", CBool(chkInativos.Value))
    lstRegistros.Clear
    Do Until rs.EOF
        lstRegistros.AddItem CStr(rs.Fields("FornecedorID").Value) & " | " & Texto(rs.Fields("NomeRazao").Value) & IIf(rs.Fields("Ativo").Value, "", " [INATIVO]")
        lstRegistros.ItemData(lstRegistros.NewIndex) = CLng(rs.Fields("FornecedorID").Value)
        rs.MoveNext
    Loop
    rs.Close
End Sub
...
Private Sub lstRegistros_Click()
    ...
    Set rs = Consultar("dbo.usp_FornecedoresObter", "@ID", lstRegistros.ItemData(lstRegistros.ListIndex))
    If rs.EOF Then Err.Raise vbObjectError + 1, , "Registro não encontrado."
    mCodigo = CLng(rs.Fields("FornecedorID").Value)
    mVersao = rs.Fields("Versao").Value
    mAtivo = CBool(rs.Fields("Ativo").Value)
    txtNomeRazao.Text = Texto(rs.Fields("NomeRazao").Value)
    txtCNPJ.Text = Texto(rs.Fields("CNPJ").Value)
...
Private Sub cmdSalvar_Click()
    Dim rs As ADODB.Recordset
    On Error GoTo Falha
    If Len(Trim$(txtNomeRazao.Text)) = 0 Then Err.Raise vbObjectError + 2, , "Informe o nome."
    cmdSalvar.Enabled = False
    Set rs = Consultar("dbo.usp_FornecedoresSalvar", "@ID", mCodigo, "@Versao", mVersao, _
        "@NomeRazao", txtNomeRazao.Text, _
        "@CNPJ", txtCNPJ.Text, _
...
Private Sub cmdSituacao_Click()
    ...
    Set rs = Consultar("dbo.usp_FornecedoresSituacao", "@ID", mCodigo, "@Ativo", Not mAtivo, "@Versao", mVersao)
    rs.Close
    NovoRegistro
    Pesquisar
    Exit Sub
